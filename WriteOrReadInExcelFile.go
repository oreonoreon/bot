...
func (U *Cache) writeCacheInFile() {
	...
	categories := map[string]string{
		"A1": "userID", "B1": "CardNumber",
		"C1": "OperationId", "D1": "Amount",
		"E1": "sum1", "F1": "sum2",
	...
	for key, value := range U.Users {
		...
		f.SetCellValue("Sheet1", k, *value.Card)
		...
		f.SetCellValue("Sheet1", k, value.Tickets[sum1])
		k, _ = excelize.CoordinatesToCellName(6, i+1)
		f.SetCellValue("Sheet1", k, value.Tickets[sum2])
		k, _ = excelize.CoordinatesToCellName(7, i+1)
		f.SetCellValue("Sheet1", k, value.Tickets[sum3])
	...
	for i := 1; i < len(rows); i++ {
		...
		for j := 1; j <= 10; j++ {
			...
			case 1:
				...
				c.FullFillCache(Id)
			case 2:
				*c.Users[Id].Card = cell
			case 3:
				*c.Users[Id].OperationId = cell
			case 4:
				cellValue, _ := strconv.ParseFloat(cell, 64)
				*c.Users[Id].Amount = cellValue
			case 5:
				cellValue, _ := strconv.Atoi(cell)
				c.Users[Id].Tickets[sum1] = cellValue
			case 6:
				cellValue, _ := strconv.Atoi(cell)
				c.Users[Id].Tickets[sum2] = cellValue
			case 7:
				cellValue, _ := strconv.Atoi(cell)
				c.Users[Id].Tickets[sum3] = cellValue
			case 8:
